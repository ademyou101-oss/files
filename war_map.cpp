...
struct FSendWarKillNotice
{
	...
	FSendWarKillNotice(const char * c_szKillerName, const char * c_szVictimName, DWORD dwKillerRace, DWORD dwVictimRace) {
...
void CWarMap::SendKillNotice(const char * c_szKillerName, const char * c_szVictimName, DWORD dwKillerRace, DWORD dwVictimRace)
{
	FSendWarKillNotice f(c_szKillerName, c_szVictimName, dwKillerRace, dwVictimRace);
	std::for_each(m_set_pkChr.begin(), m_set_pkChr.end(), f);
}
#endif
...
void CWarMap::OnKill(LPCHARACTER killer, LPCHARACTER ch)
{
	...
	if (ch->GetGuild() && killer->GetGuild())
	{
		...
		if (iRatio && iRatio % 10 == 0 && (ch->GetWarDeadCount() > ch->GetWarKillCount()))
		{
